...
def main():
    ...
    parser.add_argument('--backbone', type=str, default='transFuser',
                        help='Which Fusion backbone to use. Options: transFuser, late_fusion, latentTF, geometric_fusion, crossvit_fusion, image_only, lidar_only, no_bi_attn, no_multiscale, no_velocity, no_bi_ms, std_crossvit, no_attn, no_attn_no_ms')
    ...
    if(bool(args.use_disk_cache) == True):
        if (parallel == True):
            # 注意：这是针对我们集群设置的，数据存储在慢速存储上。
            # 训练期间将数据集缓存到本地计算节点的快速存储上。
            # 根据您的集群设置进行调整。
            # 重要：将torchrun的并行线程初始化到同一文件夹（以便共享缓存）。
            tmp_folder = str(os.environ.get('SCRATCH'))
            print("Tmp folder for dataset cache: ", tmp_folder)
            tmp_folder = tmp_folder + "/dataset_cache"
            # 使用本地磁盘缓存将数据集缓存到集群上更快的SSD驱动器
    ...
    # 使用torchrun启动，因为它有适当的错误处理，本地rank会自动设置
    if(parallel == True): # 非分布式在本地调试器中效果更好
        rank       = int(os.environ["RANK"]) # 所有进程中的rank
        local_rank = int(os.environ["LOCAL_RANK"]) # 节点上的rank
       ...
       # os.environ["CUDA_VISIBLE_DEVICES"] = str(local_rank) # 隐藏此进程不使用的设备
    ...
    torch.backends.cudnn.benchmark = True # 追求最高性能
    ...
    if (parallel == True):
        # 同步批归一化会将计算批次大小增加*num_gpus倍
        if(bool(args.sync_batch_norm) == True):
            model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
        # find_unused_parameters=True: 单模态消融实验中部分解码器参数不参与损失计算
    ...
    if ((bool(args.zero_redundancy_optimizer) == True) and (parallel == True)):
        ...
        optimizer = ZeroRedundancyOptimizer(model.parameters(), optimizer_class=optim.AdamW, lr=args.lr) # DDP训练时节省GPU内存
    else:
        optimizer = optim.AdamW(model.parameters(), lr=args.lr) # 单GPU训练
    ...
    if(parallel == True):
      ...
      dataloader_train = DataLoader(train_set, shuffle=True, batch_size=args.batch_size, worker_init_fn=seed_worker, generator=g_cuda, num_workers=8, pin_memory=True, prefetch_factor=4, persistent_workers=True)
      dataloader_val   = DataLoader(val_set,   shuffle=True, batch_size=args.batch_size, worker_init_fn=seed_worker, generator=g_cuda, num_workers=8, pin_memory=True, prefetch_factor=4, persistent_workers=True)
    ...
    if (not (args.load_file is None)):
        # 加载检查点
        print("=============load=================")
        # weights_only=True 更安全，但如果 checkpoint 包含非 tensor 对象则需设为 False
        checkpoint = torch.load(args.load_file, map_location=model.device, weights_only=False)
        # 处理DDP的"module."前缀：
        # - DDP包装后的模型期望key带有"module."前缀
        # - 如果模型已DDP包装但checkpoint没有前缀，需要添加
        # - 如果模型未DDP包装但checkpoint有前缀，需要移除
        checkpoint_has_module = any(k.startswith('module.') for k in checkpoint.keys())
        ...
        optimizer.load_state_dict(torch.load(args.load_file.replace("model_", "optimizer_"), map_location=model.device, weights_only=False))
    ...
    for epoch in range(trainer.cur_epoch, args.epochs):
        if(parallel == True):
            # 根据epoch更新种子，使分布式采样器在不同epoch使用不同的随机打乱
        ...
        if (parallel == True):
            if (bool(args.zero_redundancy_optimizer) == True):
                optimizer.consolidate_state_dict(0) # 保存整个优化器需要在GPU 0上聚合
            if (rank == 0):
                trainer.save()
        else:
            trainer.save()
...
class Engine(object):
    ...
    def load_data_compute_loss(self, data):
        ...
        if ((self.args.backbone == 'transFuser') or (self.args.backbone == 'late_fusion') or (self.args.backbone == 'latentTF') or (self.args.backbone == 'crossvit_fusion') or (self.args.backbone == 'image_only') or (self.args.backbone == 'lidar_only') or (self.args.backbone == 'no_bi_attn') or (self.args.backbone == 'no_multiscale') or (self.args.backbone == 'no_velocity') or (self.args.backbone == 'no_bi_ms') or (self.args.backbone == 'std_crossvit') or (self.args.backbone == 'no_attn') or (self.args.backbone == 'no_attn_no_ms')):
    ...
    def save(self):
        # 注意：如果使用并行处理，用torch.save(model.module.state_dict(), PATH)保存模型更简洁，保留此方式是为了向后兼容
...
# 需要单独为每个worker设置种子，否则dataloader中的随机过程在不同worker中返回相同的值！
def seed_worker(worker_id):
    # Torch初始种子在不同worker中已正确设置，需要将其传递给numpy和random
...
if __name__ == "__main__":
    # 默认的fork方法可能导致死锁
    # 使用多worker的dataloader时应使用forkserver或spawn
