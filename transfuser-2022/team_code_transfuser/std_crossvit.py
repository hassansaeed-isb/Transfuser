"""
消融实验: 标准交叉注意力 (Standard Cross-Attention)

变更说明:
    将 CrossViTBlock 的双向跨模态注意力替换为标准的单向交叉注意力:
    - 使用 PyTorch 原生 nn.MultiheadAttention 作为交叉注意力模块
    - 图像 token 作为 Query，LiDAR token 作为 Key/Value (图像查询LiDAR信息)
    - 同时保留 LiDAR→图像的对称交叉注意力
    - 但不再使用自定义的 CrossAttention 实现

    本质上这是一个使用标准库实现的 CrossViT 变体，用于验证自定义双向注意力
    相对于标准交叉注意力是否有额外收益

基于 Bi-Attenfusion.py，使用 nn.MultiheadAttention 替代自定义 CrossAttention
"""
...
class StdCrossViTBlock(nn.Module):
    """
    标准跨模态注意力块 (使用 nn.MultiheadAttention)

    与 CrossViTBlock 的区别:
        - 使用 PyTorch 原生 MultiheadAttention 实现交叉注意力
        - Q 来自一个模态，K/V 来自另一个模态
        - 保留双向交互模式 (img→lidar, lidar→img)
    """
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.):
        ...
        self.cross_attn_img2lidar = nn.MultiheadAttention(
        ...
        self.cross_attn_lidar2img = nn.MultiheadAttention(
            dim, num_heads, dropout=attn_drop, bias=qkv_bias, batch_first=True
        )
    ...
    def forward(self, img_tokens, lidar_tokens):
        # 并行双向更新: 两个方向的注意力都使用原始 tokens，避免信息泄漏
        attn_out_img, _ = self.cross_attn_img2lidar(
            self.norm1_img(img_tokens),
            self.norm1_lidar(lidar_tokens),
            self.norm1_lidar(lidar_tokens)
        )

        attn_out_lidar, _ = self.cross_attn_lidar2img(
            self.norm2_lidar(lidar_tokens),
            self.norm2_img(img_tokens),
            self.norm2_img(img_tokens)
        )

        img_tokens = img_tokens + attn_out_img
        lidar_tokens = lidar_tokens + attn_out_lidar
...
class MultiScaleStdCrossViT(nn.Module):
    """多尺度标准交叉注意力模块"""
...
class StdCrossViTBackbone(nn.Module):
    """
    消融实验Backbone: 标准交叉注意力

    使用 PyTorch 原生 nn.MultiheadAttention 实现跨模态注意力
    """
    def __init__(self, config, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=True):
        ...
        self.crossvit1 = MultiScaleStdCrossViT(
        ...
        self.crossvit2 = MultiScaleStdCrossViT(
        ...
        self.crossvit3 = MultiScaleStdCrossViT(
        ...
        self.crossvit4 = MultiScaleStdCrossViT(
