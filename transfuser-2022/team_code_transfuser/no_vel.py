"""
消融实验: 移除速度嵌入 (No Velocity Embedding)

基于 Bi-Attenfusion.py，完全移除速度嵌入模块
"""
...
class NoVelCrossViTBackbone(nn.Module):
    """
    消融实验Backbone: 移除速度嵌入
    ...
    def __init__(self, config, image_architecture='resnet34', lidar_architecture='resnet18', use_velocity=True):
        super().__init__()
        self.config = config
        self.use_velocity = False  # 消融实验: 速度嵌入已永久移除，use_velocity参数仅保留兼容性
    ...
    def forward(self, image, lidar, velocity):
        # 消融实验: velocity 参数保留用于接口兼容，但速度嵌入已永久移除
